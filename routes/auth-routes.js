...
router.post("/signup", (req, res, next) => {
  ...
  if(password.length < 7){
    console.log('Please make your password at least 8 characters long for security purposes.');
  ...
  User.findOne({ username: userNameVar }, (err, foundUser) => {
    ...
    aNewUser.save(err => {
      ...
      req.login(aNewUser, err => {
        ...
        res.status(200).json({ message: "successfully logged in", user: aNewUser });
      });
    });
  });
});
